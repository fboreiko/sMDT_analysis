...
while num < len(datacombined):
    ...
    if df.shape[0] > 500:
        ...
        ax.plot(np.arange(7), n5_eff, '^', color=datacombined[num][1], markersize=4, label='{0} chamber'.format(chamber))
        num += 1
...
plt.suptitle('Run%s_5SigmaEfficiency_forall' % (run), fontsize=20)
plt.savefig('Run%s_5SigmaEfficiency_forall' % (run))
